    ...
    this.files.forEach(function(f) {
      ...
      .map(function(filepath) {
        ...
        if (options.amd && options.namespace === false) {
          return 'return ' + compiled;
            ...
            //grunt.log.write(options.namespace);
            if (options.namespace !== false && options.amd) {
              return options.namespace+'['+JSON.stringify(filename)+'] = '+compiled+';';
            }
      ...
      if (output.length < 1) {
        ...
        if (options.amd) {
          ...
          output.unshift("var "+options.namespace+"={};");
          output.unshift("define(function(){");
          if (options.namespace !== false) {
            // Namespace has not been explicitly set to false; the AMD
            // wrapper will return the object containing the template.
            output.push("  return " + options.namespace + ";");
          }
          output.push("});");
        }
